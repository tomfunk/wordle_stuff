...
class WordleAgent1:
    ...
    def get_words(self, valid_words):
        ...
        pos_freq = {i: words[0].value_counts(normalize=True).to_dict() for i in range(5)}
    ...
    def act(self, obs):
        ...
        known_present_letters_misses = letters[scores == 1].melt().dropna().groupby('variable')['value'].apply(list).to_dict()
        ...
        if known_absent_letters:
            msk = msk & (~self.words.whole.str.contains(f'[{"".join(known_absent_letters)}]'))
            
        if known_present_letters:
            msk = msk & (self.words.whole.str.contains(f'[{"".join(known_present_letters)}]'))
